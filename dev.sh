...
echo ""
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}      Lumentra Voice AI - Dev${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""

# Check env files
if $START_API && [ ! -f "lumentra-api/.env" ]; then
    error "Missing lumentra-api/.env file!"
    exit 1
fi
...
if $START_API; then
    if $USE_DOCKER; then
        ...
        cp lumentra-api/.env .env 2>/dev/null || true
    ...
    if curl -s http://localhost:$API_PORT/health > /dev/null 2>&1; then
        ...
        [ -f /tmp/lumentra-api.log ] && tail -20 /tmp/lumentra-api.log
        exit 1
    fi
fi
...
if $START_FE; then
    ...
    npm run dev > /tmp/lumentra-dashboard.log 2>&1 &
    FE_PID=$!
    cd ..
...
$START_API && echo -e "    API:      tail -f /tmp/lumentra-api.log"
$START_FE && echo -e "    FE:       tail -f /tmp/lumentra-dashboard.log"
...
if $START_API && $START_FE; then
    ...
    tail -f /tmp/lumentra-api.log 2>/dev/null | sed 's/^/[API] /' &
    tail -f /tmp/lumentra-dashboard.log 2>/dev/null | sed 's/^/[FE]  /' &
    wait
elif $START_API; then
    if $USE_DOCKER; then
        docker compose logs -f api
    else
        tail -f /tmp/lumentra-api.log
    fi
elif $START_FE; then
    tail -f /tmp/lumentra-dashboard.log
fi
